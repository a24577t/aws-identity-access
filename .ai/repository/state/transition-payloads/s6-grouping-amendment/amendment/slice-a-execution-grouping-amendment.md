...
canonical committed identity: Git blob OID
`97faf7cb658e1238c695dcfe6ab00e2b749a10a0`, 17,938 bytes, SHA-256
`5679d79656052410e118b18a224b590e303315906c132fc8bdc92f72297557ad` (blob bytes per
the [repository byte-identity amendment](repository-byte-identity-amendment.md)
accepted alongside this record); this record supersedes exactly one thing — the form
in which `to-tickets` consumes the §10 work-item breakdown at S6. Sixteen independent
ticket lifecycles impose excessive bootstrap, review, publication, and closeout cost;
S6 instead consumes the eight-row grouping in §2 below 1:1 (one ticket per row of
this amendment). Nothing else is superseded: every §10 row, contract citation,
dependency, sequencing rule, authorization boundary, and empirical condition remains
binding exactly as accepted.
