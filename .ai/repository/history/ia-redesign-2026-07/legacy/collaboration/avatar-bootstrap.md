...
Recognize that the Collaboration Avatar below carries only durable, cross-project
...
Treat the avatar as supplementary collaboration context only, never as repository authority.
Independently reconstruct current repository state from the authoritative repository
artifacts — not from this file.
Treat the repository artifacts as authoritative: if anything here conflicts with the
repository, the repository prevails.
Not assume any project state, version, or next activity from the avatar.
Continue the collaboration using the avatar as collaboration context only.
