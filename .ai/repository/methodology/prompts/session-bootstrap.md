...
> *Example (GHES): the discovered authority domains today are **collaborator** (`.ai/collaborator/`), **repository owner** (`.ai/repository-owner/`), **repository methodology** (`.ai/repository/methodology/`: MADRs, principles, the lifecycle model, the Skill Execution Map, transition prompts), **repository state** (`.ai/repository/state/`: the Status Artifact, any Repository Continuity Artifact), and **agent configuration** (`CLAUDE.md`, `docs/agents/`). `.ai/repository/history/` is discovered as immutable evolution evidence and is excluded from required-artifact loading. Others may appear later. These paths are this repository's instantiation, not a universal assumption.*
