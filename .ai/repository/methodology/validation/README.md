...
**Subsystem charter (design document).** The artifacts under `.ai/repository/methodology/validation/` are
...
- **Accepted changes gain authority only through the repository's existing methodology
  mechanisms** — a Methodology ADR (MADR under `.ai/repository/methodology/adr/`) for an axiom or accepted
