...
- **The axioms** — the `adr/` register.
  ...
  - [`adr/0004-review-finding-persistence.md`](adr/0004-review-finding-persistence.md) (MADR-0004). Canonical persistence model for review findings: every finding that survives to gate adjudication persists at full fidelity (statement, classification, evidence, disposition, provenance) in a committed artifact within the adjudicated change set — zero-diff gates carry theirs in the reconciliation change set; authority ordering is committed record → PR/review bodies → Status Artifact summaries. Forward-looking; record conventions and tooling deferred.
